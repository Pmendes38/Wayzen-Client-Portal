...
import { createUser, getClients, getUsers } from '@/lib/queries';
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 p-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => setShowUserForm((v) => !v)} className="flex items-center gap-1.5 px-3 py-2 bg-wayzen-600 hover:bg-wayzen-700 text-white text-sm font-semibold rounded-lg transition-colors">
      ...
      {showUserForm && (
        <form onSubmit={handleCreateUser} className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-2xl p-5 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Field label="E-mail" required>
              <input type="email" className={inputCls} value={userForm.email} onChange={(e) => setUserForm({ ...userForm, email: e.target.value })} required />
            </Field>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" onClick={() => { setShowUserForm(false); setUserForm(emptyUserForm); }} className="px-4 py-2 text-sm text-gray-600 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-lg transition-colors">Cancelar</button>
